...
export function buildItemsUrl({
  ...
  filters.forEach((f, i) => {
    ...
    url += `&property[${i}][property]=${
      f.prop
    }&property[${i}][type]=${type}&property[${i}][text]=${encodeURIComponent(
      f.value
    )}`;
  });
  if (search) {
...
export async function getResourcesByTemplate(templateId, perPage = 100) {
  ...
  while (true) {
    ...
    if (!res.ok)
      throw new Error(`Failed to fetch resources for template ${templateId}`);
...
export async function fetchAllItemsForProperty(
  prop,
  maxPages = 10,
  perPage = 100
) {
  ...
  while (page <= maxPages) {
    const res = await fetch(
      `${BASE_URL}/items?page=${page}&per_page=${perPage}`
    );
    if (!res.ok) break;
    const data = await res.json();
    if (!Array.isArray(data) || data.length === 0) break;
